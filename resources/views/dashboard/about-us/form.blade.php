...
@section('content')
    @component('dashboard.layouts.includes.card', ['title' => $title])
        @slot('content')
            <form class="form" action="{{ route('admin.about.saveData') }}"
                method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-8  ">

                        {!! Form::label('image', __('lang.image') , ['class' => 'form-label'] ) !!}  (<small class="text-danger">{{ __('lang.best_size') }} : w:615 * h:430</small>)<br>
                        <img class=" image-preview-image  " width="100%" height="300"   src="{{ asset($data->image ?? 'assets/img/default.jpg' ) }}">
                        <br>
                    ...
                    <div class="col-md-6 pt-2">
                        <div class="form-group">
                            ...
                            {!! Form::text("{$locale}[title]", old("{$locale}[title]", optional($data->translate($locale))->title), ['class' => 'form-control']) !!}
                        </div>
                    </div>
                @endforeach
                ...
                @foreach (config('translatable.locales') as $key => $locale)
                    <div class="col-md-6 pt-2">
                        <div class="form-group">
                            ...
                            {!! Form::textarea("{$locale}[short_description]", old("{$locale}[short_description]", optional($data->translate($locale))->short_description), ['class' => 'form-control tinymce_ar']) !!}
                        </div>
                    </div>
                @endforeach
                ...
                @foreach (config('translatable.locales') as $key => $locale)
                    <div class="col-md-6 pt-2">
                        <div class="form-group">
                            ...
                            {!! Form::textarea("{$locale}[description]", old("{$locale}[description]", optional($data->translate($locale))->description), ['class' => 'form-control tinymce_ar']) !!}
                        </div>
                    </div>
                @endforeach
